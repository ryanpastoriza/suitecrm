<div id="mainContent" style="min-height:500px;">
	<div class="head-panel">
		<div class="row">
			<div class="col-sm-6">
				...
				<h3 class="sales-order-label">{{salesOrder.sales_order_no}}</h3>
			</div>
			<div class="col-sm-6">
				<div class="pull-right">
					<a v-if="salesOrder.status.name=='For Invoicing'" :href="invoiceURL + salesOrder.id" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-list-alt"></i> Create Invoice</a>


					<a @click="saveSo();" href="#" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-floppy-saved"></i> Save</a>
					<!-- <a href="#" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-floppy-remove"></i> Delete</a> -->
					<a @click="setStatus('For Allocation')" v-if="salesOrder.status.name=='Open' && salesOrder.docs.submitted >= salesOrder.docs.all" href="#" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-repeat"></i> Set For Allocation</a>

					<a @click="setStatus('For Invoicing')" v-if="salesOrder.status.name=='For Allocation' && salesOrder.allocated == true" href="#" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-repeat"></i> Set For Invoicing</a>
					<!-- <a :href="'index.php?module=jump_sales_invoice&action=editview&id=' + salesOrder.id" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-print"></i> Print</a> -->

					<a @click="copySo($event)" href="#" class="btn btn-default" style="margin-right:5px;"><i class="glyphicon glyphicon-saved"></i> Copy</a>
				</div>
			</div>
		</div>
	</div>
						...
						<form action="">
							<div class="form-group">
								...
								<div class="input-group"> 
								   <input v-model="salesOrder.customer.lname_c + ', ' + salesOrder.customer.fname_c + ' ' + salesOrder.customer.mname_c" type="text" class="block" disabled>
								   <span class="input-group-btn">
								        <button v-if="salesOrder.status.name=='Open'" @click="fetchCustomer()" data-toggle="modal" data-target="#customerModal" class="btn btn-danger" type="button" style="border-radius:4px;"><i class="suitepicon suitepicon-action-select"></i></button>
								   </span>
								</div>								
							</div>
							<div class="form-group">
								<label for="">Customer ID</label>
								<input v-model="salesOrder.customer_id" type="text" class="block" disabled>
							</div>
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<input v-model="salesOrder.customer.tin_c" type="text" class="block" disabled>
									</div>
								</div>
							</div>
							<div class="form-group">
								<label for="">Customer Address</label>
								<input v-model="salesOrder.customer.basic_address_c" type="text" class="block" disabled>
							</div>
						</form>
					</div>
					<div class="head">
					...
					<div class="body">
						<form action="">
							<div class="form-group">
								...
								<input v-model="salesOrder.vehicle.mmpc_model_code_c" type="text" class="block" disabled>
							</div>
							<div class="form-group">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									...
									<input v-model="salesOrder.vehicle.name" type="text" class="block" disabled>
								</div>
								<div class="form-group">
									<label for="">Preferred Color 1</label>
									<input v-model="salesOrder.preffered_color" type="text" class="block" disabled>
								</div>
								<div class="form-group">
									<label for="">Preferred Color 2</label>
									<input v-model="salesOrder.preffered_color_2" type="text" class="block" disabled>
								</div>
								<div class="form-group">
									<label for="">Preferred Color 3</label>
									<input v-model="salesOrder.preffered_color_3" type="text" class="block" disabled>
								</div>
							</div>
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<input v-model="salesOrder.vehicle.unit_price_c" type="text" class="block" disabled>
								</div>
								<div class="form-group">
							...
							<div class="col-md-12">
								<div class="vehicle-remarks">
									<div class="form-group">
										<label>Remarks</label>
										<textarea name="" id="" cols="30" rows="10" disabled>No remarks.</textarea>
									</div>
								</div>
							</div>
						</div>
	...
	<div class="panel panel-default">
		...
		<div class="panel-body panel-collapse collapse in panelContainer" id="detailpanel_-2" data-id="LBL_ACCOUNT_INFORMATION" style="padding:40px 10px;">
			<ul class="nav nav-tabs">
			    <li><a data-toggle="tab" href="#home">AUTO FINANCING</a></li>
			    ...
			    <li class="active"><a data-toggle="tab" href="#menu4">RESERVATION</a></li>
			  </ul>
			     	...
			     	<div class="row">
		      			<div class="col-md-6">
		      				<div class="form-group">
								...
								<input v-model="salesOrder.reservation_amount" type="text" class="block">
							</div>
						</div>
					</div>
			    </div>
			...
			<div class="row">
				<div class="col-md-4">
					<div class="form-group">
						...
						<input v-model="docStatus()" type="text" class="block" disabled>
					</div>
					<!-- <button class="btn btn-default"><i class="glyphicon glyphicon-plus"></i>ADD</button> -->
				...
				<table class="list view table-responsive table-striped center-table">
      				...
      				<tbody>
      					<tr v-for="docs in salesOrder.docs.all">
      						<td><input @click="docs.checked = !docs.checked" type="checkbox"></td>
      						<td>{{isSubmitted(docs)}}</td>
      						<td>{{docs.document_type}}</td>
      						<td>{{docs.name}}</td>
      						<td>{{docs.mandatory}}</td>
      						<td></td>
      						<td></td>
      						<td></td>
      						<td><button @click="submitDoc(docs)" v-if="docs.checked && isSubmitted(docs)=='No' " class="" ><i class="glyphicon glyphicon-ok"></i>SUBMIT</button></td>
      					</tr>
      					<!-- <tr>
      						<td><input type="checkbox"></td>
      						<td>No</td>
	...
	<div v-if="salesOrder.status.name!='Open'" class="panel panel-default">
		<div class="panel-heading ">
			<a class="" role="button" data-toggle="collapse" data-target="#detailpanel_-7" aria-expanded="false">
				<div class="col-xs-10 col-sm-11 col-md-11">
					Vehicle Allocation
				</div>
			</a>
		...
		<div class="panel-body panel-collapse collapse in panelContainer" id="detailpanel_-7" data-id="LBL_ACCOUNT_INFORMATION" style="padding:40px 10px;">
			<div class="row">
				...
				<table class="list view table-responsive table-striped center-table">
      				<thead>
      					...
      					<th>Site</th>
      					<th>WB#</th>
      					<th>MMPC Invoice Date</th>
      				</thead>
      				<tbody>
      					<tr>
      						<td><input type="checkbox"></td>
      						<td>{{salesOrder.vehicle.mmpc_model_code_c}}</td>
      						<td>{{salesOrder.vehicle.option_code_c}}</td>
      						<td>{{salesOrder.vehicle.model_year_c}}</td>
      						<td>{{salesOrder.vehicle.productname_c}}</td>
      						<td></td>
      						<td></td>
      						<td></td>
      						<td>{{salesOrder.preffered_color}}</td>
      						<td></td>
      						<td></td>
      						<td></td>
      					</tr>
			...
			<button v-if="!salesOrder.allocated" @click="allocateVehicle()" class="btn btn-default pull-right" data-dismiss="modal">Allocate</button>
			<h3 v-if="salesOrder.allocated" class="pull-right" style="color:#f08377;font-size:16px;font-weight:bold">Allocated</h3>
		</div>
	</div>


	<div class="status">
		<span class="pull-right">{{salesOrder.status.name}}</span>
	</div>
			...
			<div class="modal-content">
				<div class="modal-body" style="padding:0;">
					<div style="min-height:400px;">
						<table class="list view table-responsive table-striped table-custom" style="margin-bottom:16px;">
							<thead>
								<th><i class="glyphicon glyphicon-check"></i></th>
								<th>Customer ID</th>
								<th>Customer Name</th>
							...
							<tbody>
								<tr v-for="c in customers.all" @click="customers.current = c" :class="selected(c)">
									<td><center v-if="c.id_c == customers.current.id_c"><i class="glyphicon glyphicon-ok"></i></center></td>
									<td>{{c.id_c}}</td>
									<td>{{c.lname_c + ', ' + c.fname_c + ' ' + c.mname_c}}</td>
									<td>June 26, 2018</td>
								</tr>
							</tbody>
						</table>
				...
				<div class="modal-footer">
					...
					<button @click="setSelected();" class="btn btn-success pull-right" data-dismiss="modal">Select</button>
				</div>
			</div>
						<!-- /.modal-content -->	
		</div>
...
<script src="custom/include/resources/js/pnotify.js"></script>
<script>
	function show_message(type, message, title){
	  new PNotify({
	      title: title,
	      text: message,
	      type: type,
